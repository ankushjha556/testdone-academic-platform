...
async function verifyAdminFlow() {
    ...
    try {
        ...
        // 2. Create Subject
        log('2️⃣ Creating a Subject...', 'info');
        const subjectData = await request('/admin/subjects', {
            method: 'POST',
            headers: authHeaders,
            body: JSON.stringify({
                name: 'Test Verify Subject',
                slug: `test-verify-subject-${Date.now()}`,
                icon: 'Test',
                color: '#000000',
            }),
        });
        const subjectId = subjectData.data.id;
        log(`✅ Subject created: ${subjectId}`, 'success');
        ...
        } catch (e: any) { log(`   - (Optional) Could not delete section explicitly: ${e.message}`, 'info'); }

        await request(`/admin/exams/${examId}`, { method: 'DELETE', headers: authHeaders });
        log('   - Deleted Exam', 'info');

        await request(`/admin/subjects/${subjectId}`, { method: 'DELETE', headers: authHeaders });
        log('   - Deleted Subject', 'info');
